...
from config import AUTH_MODE, DISCORD_BOT_API_KEY

def require_authorization(mode_required):
    def decorator(func):
        @wraps(func)
        async def wrapper(request: Request, *args, **kwargs):
            if AUTH_MODE not in mode_required:
                print(f"Authorization mode {AUTH_MODE} not in required modes {mode_required}")
